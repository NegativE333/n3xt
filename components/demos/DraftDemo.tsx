...
export default function DraftStoryDemo() {
  ...
  // Match your HTML footer: "chars • words"
  ...
  useEffect(() => {
    ...
    const runStory = async () => {
      ...
      setStatus("Saving...");
      await typeText("Generating test payload on 2026-02-19\nID: f8a9d2b1-4c73-492a-8b1e-3f9c2d4a1e5b\n\n// Raw API Response\n", 20, 50);
      
      setStatus("Synced");
      await wait(800);

      // 4. Paste Messy JSON (The long string that was breaking layout)
      setStatus("Saving...");
      await wait(300);
...
// Raw API Response
{
  ...
  return (
    <div className="w-full max-w-[800px] mx-auto h-[600px] relative font-sans antialiased flex items-center justify-center bg-[#050509] rounded-2xl overflow-hidden shadow-2xl">
      ...
      {/* EXTENSION POPUP */}
      <div 
        className={`relative z-10 w-[500px] h-[550px] bg-[#050509] text-[#e4e4ef] border border-[#1e2030] shadow-[0_20px_80px_rgba(0,0,0,0.9)] rounded-xl overflow-hidden flex transition-all duration-700
          ${phase === 'idle' 
      ...
      >
        ...
        {/* SIDEBAR */}
        <aside className="w-[72px] bg-[#0b0b10] border-r border-[#1e2030] flex flex-col items-center py-4 gap-4 flex-shrink-0">
           <div className="text-[#8082a0] text-[18px] opacity-70">
              <span className="flex items-center justify-center w-10 h-10 rounded-full border border-[#1e2030] font-mono text-[12px] font-semibold text-[#e4e4ef]" aria-label="Draft logo">
                 N3
              </span>
           </div>
           
           <div id="tabList" className="flex-1 flex flex-col gap-2 w-full px-3 overflow-hidden items-center">
              <div className="w-12 h-12 min-h-[30px] rounded-xl bg-[#13141c] border border-[#1e2030] text-[#e4e4ef] font-mono text-[12px] tracking-[0.02em] font-semibold flex items-center justify-center cursor-pointer relative flex-shrink-0 shadow-[0_0_0_1px_#1e2030]">
                 <span className="text-center leading-none overflow-hidden text-ellipsis whitespace-nowrap max-w-full px-0.5">D1</span>
                 <div className="absolute bottom-1 right-1 w-3 h-3 flex items-center justify-center text-[#f4f4ff] opacity-90 z-10">
           ...
           <button id="btnNewTab" className="w-12 h-12 rounded-full border-[1.5px] border-dashed border-[#1e2030] bg-transparent text-[#8082a0] flex items-center justify-center cursor-pointer flex-shrink-0 mb-2">
        ...
        {/* MAIN CONTENT - Added min-w-0 here to prevent horizontal overflow */}
        <main className="flex-1 flex flex-col bg-[#0f1015] min-w-0">
           
           {/* HEADER */}
           <header className="h-[52px] border-b border-[#1e2030] flex items-center justify-between px-5 bg-[#0f1015] shrink-0 min-w-0">
              <div className="flex items-baseline gap-3 flex-1 min-w-0">
                 <div className="text-[14px] font-semibold cursor-pointer whitespace-nowrap overflow-hidden text-ellipsis max-w-[200px] leading-[1.3]">
                    Draft 1
                 </div>
                 <div className="text-[10px] text-[#8082a0] font-mono opacity-60 whitespace-nowrap font-normal">
                    just now
                 </div>
              </div>
              
              <div className="flex items-center gap-3 shrink-0">
                 <div className={`text-[11px] font-mono transition-colors duration-300
                    ${status === 'Saving...' ? 'text-[#eab308]' : status === 'Formatted' ? 'text-[#10b981]' : 'text-[#8082a0]'}
                 `}>
                    {status}
                 </div>
                 
                 <button className="w-8 h-8 rounded-full border border-transparent bg-transparent text-[#8082a0] flex items-center justify-center text-[16px]">
           ...
           {/* EDITOR CONTAINER - Added min-h-0 and min-w-0 to fix flexbox overflow */}
           <div className="flex-1 bg-[#0f1015] p-4 relative min-w-0 min-h-0">
              {content.length === 0 && (
                 <div className="absolute top-[34px] left-[36px] text-[#8082a0] font-mono text-[13px] pointer-events-none z-10">
                    Drop thoughts, JSON, snippets... anything.
                 </div>
              )}
              
              {/* Added overflow-y-auto, overflow-x-hidden, and break-all */}
              <div 
                 className="w-full h-full rounded-xl bg-[#13141c] text-[#e4e4ef] p-[18px_20px] font-mono text-[13px] leading-[1.7] whitespace-pre-wrap outline-none break-all overflow-y-auto overflow-x-hidden"
                 style={{ 
              ...
              >
                 {content}
                 <span className={`inline-block w-0.5 h-4 bg-[#e4e4ef] ml-[1px] align-middle transition-opacity duration-100 ${showCursor ? 'opacity-100' : 'opacity-0'}`} />
              </div>
           </div>

           {/* FOOTER */}
           <footer className="h-[48px] border-t border-[#1e2030] flex items-center justify-between px-5 bg-[#0f1015] gap-3 shrink-0 min-w-0 overflow-hidden">
              <div className="text-[11px] text-[#8082a0] font-mono whitespace-nowrap flex-[0_1_auto] min-w-0 overflow-hidden text-ellipsis">
                 {charCount} chars • {wordCount} words
              </div>
              
              <div className="flex items-center gap-[6px] flex-shrink-0">
                 <button 
                    className={`rounded-full border border-transparent px-3 py-1.5 text-[12px] font-medium flex items-center gap-[5px] transition-all duration-150
                       ${isFormatHovered ? 'bg-[rgba(244,244,255,0.06)] text-[#f4f4ff] border-[#1e2030]' : 'bg-transparent text-[#8082a0]'}
                    `}
                 >
                    <span className="text-[11px]">{`{ }`}</span> Format
                 </button>
                 <div className="w-px h-5 bg-[#1e2030] opacity-60"></div>
                 <button className="rounded-full border border-transparent bg-transparent px-3 py-1.5 text-[12px] font-medium text-[#8082a0] flex items-center gap-[5px]">
                    Copy
                 </button>
                 <button className="rounded-full border border-transparent bg-transparent px-3 py-1.5 text-[12px] font-medium text-[#8082a0] flex items-center gap-[5px]">
                    Clear
